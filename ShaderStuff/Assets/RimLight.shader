Shader "Toon/Rim Outline" {
	...
	v2f vert(appdata v) {
		...
		float2 offset = TransformViewToProjection(outline) + (float2(o.lightDir.x, -o.lightDir.y) * 2);// add the light direction to the offset for rimlight only on the light side
